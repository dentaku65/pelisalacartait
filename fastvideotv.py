...
def mainlist(item):
    logger.info("pelisalacarta.fastvideotv mainlist")
    itemlist = []
...
def categorias(item):
    ...
    for scrapedurl,scrapedtitle,scrapedplot in matches:
        ...
        itemlist.append( Item(channel=__channel__, action="peliculas", title="[COLOR azure]"+scrapedtitle+"[/COLOR][COLOR gray]" +scrapedtot+"[/COLOR]" , url=scrapedurl ,  plot=scrapedplot , thumbnail="http://xbmc-repo-ackbarr.googlecode.com/svn/trunk/dev/skin.cirrus%20extended%20v2/extras/moviegenres/All%20Movies%20by%20Genre.png", folder=True) )
...
def peliculas(item):
    logger.info("pelisalacarta.fastvideotv peliculas")
    itemlist = []
